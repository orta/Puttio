...
- (UIFont *)labelFont
{
	return [UIFont fontWithName:@"Futura-CondensedMedium" size:24];
}

static float borderWidth = 2;
static float textOffset = 1;
...
//		UIBezierPath *bezierPath = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(-self.frame.origin.x + 0.5, 0, self.bounds.size.width / 2.0 + self.bounds.size.height / 2.0 - 1.5, self.bounds.size.height) cornerRadius:self.bounds.size.height / 2.0];
        UIBezierPath *bezierPath = [UIBezierPath bezierPathWithRect:CGRectMake(-self.frame.origin.x + 0.5, 0, self.bounds.size.width / 2.0 + self.bounds.size.height / 2.0 - 1.5, self.bounds.size.height)];
		CGContextAddPath(context, bezierPath.CGPath);
		CGContextClip(context);
	}

    // border
    CGContextSetFillColorWithColor(context, [UIColor putioBlue].CGColor);
...
//	[self.onString drawAtPoint:onTextPoint withFont:self.labelFont];
	...
	CGPoint offTextPoint = CGPointMake(textSpaceWidth + (textSpaceWidth - offTextSize.width) / 2.0 + knobRadius * .86, floorf((self.bounds.size.height - offTextSize.height) / 2.0));
	[[UIColor putioRed] set];
	[self.offString drawAtPoint:CGPointMake(offTextPoint.x, offTextPoint.y  + textOffset) withFont:self.labelFont];
